...
import { getAppSettings, setDigestEmailHourUtc } from "../../src/settings/appSettings";
...
// database - save/restore both it and the configured digest hour so this
...
describe("daily digest email tick", () => {
  ...
  beforeAll(async () => {
    ...
    originalHour = (await getAppSettings()).digestEmailHourUtc;
  });

  afterAll(async () => {
    await db.updateTable("digest_email_state").set({ last_sent_date: originalLastSentDate }).where("id", "=", 1).execute();
    await setDigestEmailHourUtc(originalHour);
    await closeDb();
  });

  it("is a no-op outside the configured hour", async () => {
    const currentHour = new Date().getUTCHours();
    await setDigestEmailHourUtc((currentHour + 12) % 24); // guaranteed not "now"
  ...
  it("sends and records today's date once the configured hour is reached", async () => {
    const now = new Date();
    await setDigestEmailHourUtc(now.getUTCHours());
    await db.updateTable("digest_email_state").set({ last_sent_date: null }).where("id", "=", 1).execute();
